...
from pox.openflow.discovery_new import LLDPSender, Link, LinksHandler  # EDIT MESH TEAM
...
class SDN_Server:
    def __init__(self, port, link_handler, max_clients=3):
        self.sock = None
        self.port = port
        self.link_handler = link_handler
        self.max_clients = max_clients
        self.clients = set()
        self.pdu = {"Header": "MT", "MessageType": "Inform", "DataType": "Links", "Data": {}}
    ...
    def start(self):
        ...
        Thread(target=self.connect_clients).start()
        Thread(target=self.send_links_to_clients).start()
    ...
    def send_links_to_clients(self):
        while True:
            links_data = self.pdu.copy()
            links_data["Data"]["SWITCHES_LINKS"] = self.link_handler.link_id_set
            links_data["Data"]["NO_FLOOD_LINKS"] = self.link_handler.flood_blocked_links
            links_data["Data"]["SWITCHES_DPIDS"] = list(self.link_handler.dpids.keys())

            links_data = pickle.dumps(links_data)

            loop_list = list(self.clients)
            for client in loop_list:
                try:
                    client.sendall(links_data)
                except Exception as e:
                    print(f"Error sending PDU to client: {e}")
                    print("Removing client from clients list")
                    try:
                        self.clients.remove(client)
                    except Exception as e:
                        print("Error removing client from clients list: ", e)
...
class LearningSwitch(object):
  ...
  """
    ...
    def lldp_packets_handler(self, event, packet):
        ...
        if event.port == 65535:
            ...
            if packet.src in totalMacToPort.keys():
                ...
                if len(self.linkHandler.lldp_info.disconnected_links):
                    print("Disconnected links:", self.linkHandler.lldp_info.disconnected_links)
                    loop_links = self.linkHandler.lldp_info.disconnected_links.copy()
                    for link in loop_links:
                        self.linkHandler.handle_disconnected_link(link)
    ...
    """
        ...
        #     print("packet received at dpid:", event.dpid, "port:", event.port, "packet:",
        #           hex(packet.effective_ethertype),
...
class l2_learning(object):
    ...
    def set_lldp_rules_on_switch(self, connection):
        # when receiving LLDP packet from the controller, send it to all ports.
        msg = of.ofp_flow_mod()
        # msg.priority = 65535
...
def launch(transparent=False, hold_down=_flood_delay, ignore=None):
  ...
  """
    ...
    LLDPSender(8)
    SDN_Server(7777, LinkHandle).start()
